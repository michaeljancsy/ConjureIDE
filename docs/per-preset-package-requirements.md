...
Before every Python script load, the AU calls `dsp_kernel_set_extra_site_packages(kernel, path)` to set the `user-packages/` directory. The Rust kernel prepends this to `sys.path` before `PyModule::from_code()`, so user-installed packages are importable by any preset.
...
5. Companion app runs: `uv pip install --target <user-packages> --python <bundled> <packages>`
...
Same flow but with `package-uninstall-request.json`. Companion app removes package directory and `.dist-info`.
...
| File | Purpose |
|------|---------|
| `rust/conjure_dsp/src/python_backend.rs` | Accept `extra_site_packages` param, inject into sys.path |
| `rust/conjure_dsp/src/kernel.rs` | Store `extra_site_packages` field, pass to PythonBackend |
...
| `ConjureDSPExtension/Common/Audio Unit/ConjureDSPExtensionAudioUnit.swift` | Call `set_extra_site_packages` before every script load |
