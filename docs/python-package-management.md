...
| Shared runtime at `~/Library/Application Support/BearBone/PythonRuntime-3.14/` | Already used by exported AUs |
...
│   runs bundled `uv`       │           │   "Open BearBone app to install"│
│   code-signs .so files    │           └──────────────────────────────────┘
└───────────────────────────┘
          │
          ▼
~/Library/Application Support/BearBone/PythonRuntime-3.14/
...
- **Add** "Copy uv Binary" run script build phase on **BearBone host app target**
- Copies `uv-dist/uv` → `$BUILT_PRODUCTS_DIR/BearBone.app/Contents/Resources/uv-dist/uv`
- Code-signs the binary
- Auto-symlinks `uv-dist/` from main worktree (same pattern as python-dist)
- **Modify** `BearBone.xcodeproj/project.pbxproj`

### Step 3: Add `extra_site_packages` to Rust kernel
- **Modify** `rust/bearbone_dsp/src/kernel.rs`
...
- **Modify** `rust/bearbone_dsp/src/python_backend.rs`
...
- **Modify** `rust/bearbone_dsp/src/lib.rs`
  - Add FFI: `dsp_kernel_set_extra_site_packages(kernel, path: *const c_char) -> bool`

### Step 4: Wire Swift → Rust
- **Modify** `BearBoneExtension/Common/Audio Unit/BearBoneExtensionAudioUnit.swift`
  - In `loadPythonScript()`, after resolving pythonHome, call:
    ...
    ```
- **Modify** `BearBone/Model/SharedPythonRuntimeInstaller.swift`
...
- **Create** `BearBone/Model/PackageInstaller.swift`
...
- After install, walks user-packages for `.so`/`.dylib` files and ad-hoc signs them (follows `PendingExportHandler.codeSign()` pattern at `BearBone/Model/PendingExportHandler.swift:119`)
...
- **Create** `BearBone/Views/PackageManagerView.swift`
...
- **Modify** `BearBone/ContentView.swift` — add toolbar button
- **Modify** `BearBone/BearBoneApp.swift` — instantiate PackageInstaller if needed
...
- Factory preset: `BearBoneExtension/Resources/preset_delay.py` → `preset_delay-requirements.txt`

### Step 7: Requirements file discovery
- **Modify** `BearBoneExtension/Model/PresetManager.swift`
...
- **Modify** `BearBoneExtension/UI/BearBoneExtensionMainView.swift`
  - When error contains `ModuleNotFoundError` or `No module named`, append guidance:
    "Open BearBone app → Packages to install it."

### Step 9: AI integration
- **Modify** `BearBoneExtension/AI/AnthropicProvider.swift`
...
| `.gitignore` | Modify | 2A |
| `BearBone.xcodeproj/project.pbxproj` | Modify | 2A |
| `rust/bearbone_dsp/src/kernel.rs` | Modify | 2A |
| `rust/bearbone_dsp/src/python_backend.rs` | Modify | 2A |
| `rust/bearbone_dsp/src/lib.rs` | Modify | 2A |
| `BearBoneExtension/Common/Audio Unit/BearBoneExtensionAudioUnit.swift` | Modify | 2A |
| `BearBone/Model/SharedPythonRuntimeInstaller.swift` | Modify | 2A |
| `BearBone/Model/PackageInstaller.swift` | Create | 2B |
| `BearBone/Views/PackageManagerView.swift` | Create | 2B |
| `BearBone/ContentView.swift` | Modify | 2B |
| `BearBoneExtension/Model/PresetManager.swift` | Modify | 3 |
| `BearBoneExtension/UI/BearBoneExtensionMainView.swift` | Modify | 3 |
| `BearBoneExtension/AI/AnthropicProvider.swift` | Modify | 3 |
...
2. **Shared runtime versioning** — If BearBone updates Python (3.14→3.15), user packages are lost. Consider migration strategy.
