# ConjureDSP Preset Repo Format

This document specifies the structure of a GitHub repository that ConjureDSP can connect to for browsing, syncing, and sharing DSP preset scripts.
...
```
conjuredsp.json                        ← required marker file
...
### `conjuredsp.json` (required)

A marker file at the repository root that identifies the repo as a ConjureDSP preset repo. ConjureDSP checks for this file when connecting to an existing repo and rejects repos without it.
...
Both directories are optional — a repo may contain only Python presets, only Rust presets, or both. ConjureDSP lists the contents of each directory to discover presets.
...
Users can connect an existing repo or have ConjureDSP create one via Settings → GitHub. When ConjureDSP creates a repo, it initializes it with `conjuredsp.json` and empty `python/` and `rust/` directories.
...
- **On launch**, ConjureDSP runs a bidirectional sync: pulling new remote presets and pushing new local ones, with per-file conflict resolution for divergent changes.

### Validation

When connecting to an existing repo, ConjureDSP fetches `conjuredsp.json` from the repo root and verifies:
