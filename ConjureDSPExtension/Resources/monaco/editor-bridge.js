...
const bridge = {
    ...
    _registerCompletions() {
        ...
        const dspFunctions = [
            ...
            { name: 'equal_power_crossfade', pySig: '(dry, wet, mix, out, n)', rustSig: '(dry: f32, wet: f32, mix: f32) -> f32', doc: 'Equal-power crossfade using sine/cosine curves. Preserves energy at 50% mix.', insert: 'equal_power_crossfade(${1:dry}, ${2:wet}, ${3:mix}, ${4:out}, ${5:n})', params: [{ label: 'dry', doc: 'Dry buffer' }, { label: 'wet', doc: 'Wet buffer' }, { label: 'mix', doc: 'Mix (0–1)' }, { label: 'out', doc: 'Output buffer' }, { label: 'n', doc: 'Frame count' }], rustParams: [{ label: 'dry', doc: 'Dry sample' }, { label: 'wet', doc: 'Wet sample' }, { label: 'mix', doc: 'Mix amount (0–1)' }] },
        ];
        ...
        dspFunctions.forEach(f => {
            ...
            if (f.rustSig) rustSignatures[f.name] = { label: f.name + f.rustSig, doc: f.doc, params: f.rustParams || f.params };
        });
        ...
        rustHoverDocs['Context'] = '```rust\nstruct Context\n```\nSafe wrapper for audio buffer access. Built by `process! { ctx => … }` and bound to the user-named identifier on the left of `=>`.\n\nMethods: `channels()`, `frames()`, `sample_rate()`, `input(ch, fr)`, `set_output(ch, fr, val)`, `param(idx)`, `sidechain(c, i)`, `sidechain_connected()`, `set_telemetry_scalar(slot, value)`';
            ...
            provideCompletionItems(model, position) {
                ...
                return { suggestions: [
                    ...
                    sug('process', Kind.Snippet,
                        ...
                        'Entry point for every Rust preset. `process!` emits the zero-arg `extern "C" fn process()` and the buffer statics — do NOT write `setup!()` separately.\n' +
                        'The identifier on the left of `=>` becomes the `Context` binding (`ctx` is canonical).',
                        true),
                    ...
                    sug('DelayLine', Kind.Snippet,
                        [
                            ...
                            '\t\t\tlet wet = d[c].read(${3:delay_samples} as f64);',
                            '\t\t\tctx.set_output(c, i, wet);${0}',
                            '\t\t}',
                            '\t}',
                            '});',
                        ].join('\n'),
                    ...
                    sug('state (scalar)', Kind.Snippet,
                        [
                            '// Persistent scalar state across blocks',
                            'persist!(${1:ENVELOPE}: f64 = 0.0);',
                            '',
                            '// Read/write:',
                            '${1:ENVELOPE}.set(${1:ENVELOPE}.get() * 0.95 + ${2:input} * 0.05);',
                        ].join('\n'),
                        'Scalar persistent state via `persist!`. Use f64 for precision in feedback loops.',
                        true),
                    ...
                    sug('delay line', Kind.Snippet,
                        [
                            ...
                            '',
                            '// Inside process! { ctx => ... }:',
                            'let delay_ms = ctx.param(${1:TIME});',
                            'let feedback = ctx.param(${2:FEEDBACK});',
                            'let mix = ctx.param(${3:MIX});',
                            ...
                            '\t\t\tlet delayed = buf[c][rp];',
                            '\t\t\tbuf[c][wp] = dry + delayed * feedback;',
                            '\t\t\tctx.set_output(c, i, dry * (1.0 - mix) + delayed * mix);',
                            '\t\t}',
                            '\t\twp = (wp + 1) % MAX_DELAY;',
                            '\t}',
                            '});',
                    ...
                    sug('biquad', Kind.Snippet,
                        [
                            ...
                            'let cutoff_hz = ctx.param(${1:CUTOFF}) as f64;',
                            ...
                            '',
                            'BIQ_STATE.with_mut(|s| {',
                            '\tfor c in 0..ctx.channels() {',
                            '\t\tlet (mut x1, mut x2, mut y1, mut y2) = (s[c][0], s[c][1], s[c][2], s[c][3]);',
                            '\t\tfor i in 0..ctx.frames() {',
                            '\t\t\tlet x0 = ctx.input(c, i) as f64;',
                            '\t\t\tlet y0 = b0*x0 + b1*x1 + b2*x2 - a1*y1 - a2*y2;',
                            '\t\t\tctx.set_output(c, i, y0 as f32);',
                            '\t\t\tx2 = x1; x1 = x0; y2 = y1; y1 = y0;',
                            '\t\t}',
                            '\t\ts[c] = [x1, x2, y1, y2];',
                            '\t}',
                    ...
                    sug('envelope follower', Kind.Snippet,
                        [
                            ...
                            '// Inside process! { ctx => ... }:',
                            'let sr = ctx.sample_rate() as f64;',
                            'let attack_ms = ctx.param(${1:ATTACK}) as f64;',
                            'let release_ms = ctx.param(${2:RELEASE}) as f64;',
                        ...
                        'Peak-detecting envelope follower with attack/release using `persist!`.\n' +
                    ...
                    sug('soft clip (tanh)', Kind.Snippet,
                        [
                            '// Soft clipping via tanh saturation, inside process! { ctx => ... }',
                            'let drive = ctx.param(${1:DRIVE});',
                            'for c in 0..ctx.channels() {',
                            '\tfor i in 0..ctx.frames() {',
                            '\t\tctx.set_output(c, i, (ctx.input(c, i) * drive).tanh());',
                            '\t}',
                            '}',
                        ].join('\n'),
