...
persist_mut!(PD: [DelayLine<MAX_DL>; 2] = [const { DelayLine::new() }; 2]);
persist_mut!(CB: [[DelayLine<MAX_DL>; 4]; 2] = [const { [const { DelayLine::new() }; 4] }; 2]);
persist_mut!(CLP: [[Biquad; 4]; 2] = [const { [const { Biquad::new() }; 4] }; 2]);
...
persist_mut!(LFOS: [Lfo; 4] = [const { Lfo::new() }; 4]);
