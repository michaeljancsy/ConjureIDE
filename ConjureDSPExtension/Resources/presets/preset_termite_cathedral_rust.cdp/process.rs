...
persist_mut!(GRAIN_DL: [DelayLine<MAX_DL>; 2] = [const { DelayLine::new() }; 2]);
persist_mut!(TAP_LFO: [Lfo; 8] = [const { Lfo::new() }; 8]);
persist_mut!(RES: [[Biquad; 4]; 2] = [const { [const { Biquad::new() }; 4] }; 2]);
