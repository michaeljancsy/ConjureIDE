...
persist_mut!(MODAL: [[Biquad; 6]; 2] = [const { [const { Biquad::new() }; 6] }; 2]);
persist_mut!(SHIMMER_DL: [DelayLine<MAX_DL>; 2] = [const { DelayLine::new() }; 2]);
persist_mut!(GRAN_DL: [DelayLine<MAX_DL>; 2] = [const { DelayLine::new() }; 2]);
persist_mut!(SUB_LP: [Biquad; 2] = [const { Biquad::new() }; 2]);
persist_mut!(SHIM_HP: [Biquad; 2] = [const { Biquad::new() }; 2]);
