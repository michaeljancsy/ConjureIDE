...
extension PresetBundle {
    ...
    ///
    /// When `audioFrames` is true, the scaffold also includes a
    /// `<canvas id="spec">`, a rAF-coalesced redraw helper, and the
    /// peak-hold-then-decay-toward-floor smoothing pattern that turns
    /// the ~23 Hz FFT column rate into smooth display-rate motion.
    /// Authors who don't want a spectrum should leave
    /// `manifest.ui.audioFrames = false` so they never see this block.
    static func starterIndexHTML(audioFrames: Bool = false) -> String {
        ...
        let spectrumScript = audioFrames ? """
          ...
          <script>
            ...
            function draw() {
              // First frames haven't landed yet — nothing to paint.
              ...
              // Placeholder visualization: one filled bar per spectrum at
              // the mean dB across bins. Replace with whatever you want —
              // per-bin bars, filled paths, log-frequency mapping, etc.
              // preset_eq3.cdp/ui/index.html has a worked log-x example.
            ...
            if (CDP && CDP.audio && CDP.audio.onFrame) {
              CDP.audio.onFrame((frame) => {
                ...
                // halfN bins span 0..Nyquist; bin i maps to f = i * SR / fftSize.
                // Never hard-code 48000 — the host stamps the active sample
                // rate onto every frame so the axis stays correct at non-48k.
