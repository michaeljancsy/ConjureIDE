...
class PresetManager: ObservableObject {
    ...
    /// Fixed warning written to `_paramsNote` on every non-empty manifest
    /// param sync. JSON has no comment syntax, so this sibling string is
    /// the closest equivalent — it sorts above `params` in the
    /// alphabetically-keyed pretty-print so it reads as a header.
    static let kernelDerivedParamsNote = "Auto-generated by ConjureDSP on save. Edit PARAMS / params!() in your script — changes here will be overwritten."

    /// Overwrite a user bundle's `manifest.json` `params` block (and the
    /// sibling `_paramsNote`) from the kernel's freshly-extracted
    /// metadata. Every other field is preserved — including top-level
    /// keys outside `PresetManifest`'s schema.
    ///
    /// Implemented as a JSON-tree mutation, not a `Codable` round-trip,
    /// because Codable's default decode silently ignores unknown keys
    /// and re-encode would drop them on the way back out. With ~3
    /// active users hand-editing manifests, that silent drop has
    /// already cost (real incident: a hand-added `"metadata": "asdf"`
    /// vanished on the next load). Mutating the parsed JSON dict
    /// preserves any extra top-level field the author added.
    ///
    /// Always overwrites the `params` block — `manifest.params` is a
    /// cache of kernel metadata, not an authoritative override. Any
    /// hand-edits to that specific block are lost on the next sync.
    /// The `_paramsNote` string surfaces that contract in the file so
    /// authors who try to hand-edit `params` see the warning before
    /// their change is silently overwritten.
    ///
    /// Behavior:
    ///  - `params` non-empty: write the new array and set `_paramsNote`
    ///    to the fixed warning. Returns true (false on idempotent skip).
    ///  - `params` empty: remove the manifest's `params` and
    ///    `_paramsNote` keys — covers the "user removed PARAMS dict
    ///    from their script" case. Returns true if either was
    ///    previously set; false if the manifest is already clean.
    ///  - Factory bundles (read-only Resources): skip silently. Returns
    ///    false. Leaving the gate in place is load-bearing — a future
    ///    "cleanup" that drops it would let `save_preset` against a
    ///    factory bundle attempt a write that would fail at the OS
    ///    level under the hardened runtime.
    ///  - Manifest fails to parse as JSON at all: skip the write so
    ///    we don't clobber the on-disk file — the user can still see
    ///    and fix their syntax error. Returns false.
    @discardableResult
    func syncManifestParamsFromKernel(
        ...
        guard let existingData = try? Data(contentsOf: manifestURL),
            ...
            // Unparseable JSON. Don't synthesize a replacement — that
            // would silently destroy whatever the user typed. Better to
            // leave it alone so the in-app validator (or external
            // editor) can surface the error.
            return false
        }

        // Encode the kernel-derived params via the same JSONEncoder
        // settings PresetManifest uses, then re-parse as a JSON array
        // of dicts so we can splice it into the larger JSON tree.
        let newParamsArray: [Any]
        if params.isEmpty {
            newParamsArray = []
        } else {
            ...
            newParamsArray = (try? JSONSerialization.jsonObject(with: encoded)) as? [Any] ?? []
        }

        if params.isEmpty {
            let hadParams: Bool = {
                guard let arr = json["params"] as? [Any] else { return false }
                return !arr.isEmpty
            }()
            let hadNote = json["_paramsNote"] != nil
            if !hadParams && !hadNote { return false }
            json.removeValue(forKey: "params")
            json.removeValue(forKey: "_paramsNote")
        } else {
            // Idempotency check: decode the existing `params` block
            // back into `[ParamDecl]` and compare via Equatable. Beats
            // raw NSArray equality because field ordering inside each
            // dict varies depending on which side wrote the JSON.
            let existingDecoded: [PresetManifest.ParamDecl] = {
                guard let existingParamsAny = json["params"],
                      let existingParamsData = try? JSONSerialization.data(withJSONObject: existingParamsAny),
                      let decoded = try? JSONDecoder().decode([PresetManifest.ParamDecl].self, from: existingParamsData)
                else { return [] }
                return decoded
            }()
            let existingNote = json["_paramsNote"] as? String
            if existingDecoded == params, existingNote == Self.kernelDerivedParamsNote {
                return false
            }
            json["params"] = newParamsArray
            json["_paramsNote"] = Self.kernelDerivedParamsNote
        }
