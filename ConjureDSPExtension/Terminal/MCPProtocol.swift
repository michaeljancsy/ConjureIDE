...
enum MCPProtocol {
    ...
    static let tools: [ToolDefinition] = [
        ...
        ToolDefinition(
            name: "get_error",
            description: "Get the last error from script loading, compilation, OR a runtime WASM/Python trap during audio processing. Response shape: `{error: string | null, source?: \"compile\" | \"runtime\"}`. Compile errors are preferred when both are present (freshest signal). Runtime traps survive across calls until the next successful render (audio thread auto-clears on recovery) or the next compile_and_run / save_preset. If a script compiles cleanly but the post-load benchmark hits a trap on its synthetic-audio runs, compile_and_run already surfaces that inline as `runtime_trap` — use this tool when you didn't capture that response or when checking for traps that surfaced during a live render between MCP calls.",
            inputSchema: InputSchema(type: "object", properties: [:], required: nil)
        ),
        ToolDefinition(
            name: "set_parameter",
        ...
        ToolDefinition(
            ...
            inputSchema: InputSchema(
                ...
                properties: [
                    ...
                    "ui_height": PropertySchema(type: "integer", description: "Height in points for the scaffolded ui block. Same semantics as `ui_width`. Defaults to 380 when omitted.", minimum: 120, maximum: 1600),
                    "ui_fps": PropertySchema(type: "integer", description: "Frame rate (Hz) for the scaffolded ui block's render loop. Only honored when `scaffold_ui` is true and no manifest.ui block exists yet. Default depends on `ui_audio_frames`: 30 for pure-control UIs (the common case), 60 when `ui_audio_frames: true` (canvas-driven scope / spectrum / meter UIs benefit from the smoother refresh). Pass an explicit value to override either default.", minimum: 1, maximum: 120),
                    "ui_audio_frames": PropertySchema(type: "boolean", description: "When true, the scaffolded ui block opts into the audio frame stream — required if the UI will use `<cdp-meter>`, `<cdp-scope>`, `<cdp-bargraph>`, or call `audio.onFrame(...)` directly. Defaults to false (pure-control UIs are the common case). Only honored when `scaffold_ui` is true and no manifest.ui block exists yet. When true, `ui_fps` also defaults to 60 instead of 30 unless overridden."),
                ],
                required: ["name"]
            )
        ),
        ...
        ToolDefinition(
            name: "dsp_probe",
            description: "Render the currently loaded DSP script offline against a synthesized test signal and report time-domain stats (RMS, peak, DC, NaN/Inf) plus, for sine probes, a spectral pitch-shift check. Catches audio bugs neither validate_bundle nor smoke_test_ui can see: NaN propagation, broken passthrough at identity settings, gross gain/polarity errors, and unintended pitch-shift / time-warp (e.g. a granular/delay preset whose `Forward`/identity pattern doesn't actually pass audio through verbatim). After the probe, the script is reloaded under a brief mute envelope to reset DSP state (filter histories, delay buffers, LFO phase) so reverb/delay tails don't carry test-signal residue. Use after compile_and_run to verify behavior. Response shape: `{signal, sample_rate, channel_count, frames, block_size, in_rms, out_rms, in_peak, out_peak, out_dc, has_nan, has_inf}` plus `pitch_shift_ratio` (sine signals only, when the probe is at least 4096 samples and the output isn't silent) plus `fallback_reason` (only present when the kernel hit passthrough fallback during the probe — i.e. the WASM/Python script trapped and the kernel substituted bit-exact input→output copy. When you see this field, the probe's RMS/peak telemetry will look like passthrough regardless of what the script *intended* — diagnose by reading the trap message, not the audio stats). `pitch_shift_ratio` is `output_dominant_freq / input_dominant_freq`: 1.0 = pitch-preserving (the only correct answer for a preset at identity / unity settings), 2.0 = octave up, 0.5 = octave down, 1.5 = perfect fifth up, etc. If your preset is supposed to pass audio through unchanged at default params and you see a ratio noticeably off from 1.0, the DSP is incorrect — verify your read-pointer math. Note: the audio thread Mutex-contends on the kernel during the probe; if a DAW is actively playing through the AU, expect brief glitches in the user's audio and possibly polluted reverb/delay tails until the post-probe reload completes.",
            inputSchema: InputSchema(
                type: "object",
                properties: [
