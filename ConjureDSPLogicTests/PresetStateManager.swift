...
/// Coordinator for the bundle-private STATE channel.
///
...
///      with the kernel's bytes so `_init` payloads, JS bridge writes,
///      and MCP reads can operate on a structured view without parsing
///      JSON every time.
///   3. The DAW project (or user preset bundle) — restored via the AU's
///      `fullState` / `fullStateForDocument` setters at load time.
///
...
///
/// `@MainActor` because every consumer (custom UI WKWebView, MCP
/// dispatch, AU's fullState getter/setter) runs on main. The kernel
/// FFI is itself thread-safe by atomic-swap; the actor isolation is
/// purely to keep the Swift mirror coherent.
@MainActor
final class PresetStateManager {
    /// Subject fired whenever state changes via any path (set, reset,
    /// restore from fullState, MCP write). Used by `CustomUIWebView` to
    /// forward `_stateUpdate` to JS, and by tests to assert mutations.
    ...
    struct StateChange {
        ...
        /// origins (DAW load, MCP, preset switch) propagate through.
        let origin: Origin
    }

    enum Origin: Equatable {
        case ui          // JS bridge `state.set` — suppress echo to that webview.
        case mcp         // MCP `set_state` / `reset_state`.
    ...
    /// by MCP `set_state` to reject unknown keys (strict mode) and by
    /// the validator to resolve `ConjureDSP.state.get/set` references.
    ...
    /// is the set of top-level keys the script declared, used for
    /// validator + MCP strictness.
    ///
    ...
    func set(key: String, value: Any?, origin: Origin) -> Bool {
        ...
        guard let bytes = jsonBytes(newMirror) else {
            log.error("PresetStateManager: failed to serialize state mirror")
            return false
        }
        guard bytes.count <= maxBytes else {
            log.error(
                "PresetStateManager: state size \(bytes.count) exceeds cap \(self.maxBytes, privacy: .public)"
            )
            return false
        }
        ...
        if !ok {
            log.error("PresetStateManager: kernel rejected state write (cap or JSON)")
            return false
        }
        mirror = newMirror
        stateChanges.send(StateChange(key: key, value: value, origin: origin))
    ...
    func restore(from bytes: Data) {
        ...
        if !ok {
            log.warning(
                "PresetStateManager.restore: kernel rejected stored state (\(bytes.count) bytes); falling back to script defaults"
            )
    ...
    /// Current state generation. Used by tests + the smoke tester to
    /// verify a write actually advanced the kernel.
    ...
    private func jsonBytes(_ dict: [String: Any]) -> Data? {
        ...
        // identical mutation re-serializes in a different order. The
        // kernel itself doesn't care, but the test harness compares
        // bytes.
