...
//
//  Audio-thread-safe counters for render-block performance. The hot path
//  (`recordRender`) performs only lock-free atomic ops implemented in
//  ExportRenderStatsAtomics.h — no allocations, no Swift runtime calls beyond
//  a handful of integer loads/stores, no locks.
//
//  Owned by ExportAUAudioUnit. The main thread polls `snapshot()` at 1 Hz from
//  the existing error-polling timer in ExportParameterState.
//
