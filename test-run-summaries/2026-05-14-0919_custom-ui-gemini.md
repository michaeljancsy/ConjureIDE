...
- [skill] **(resolved by [PR #332](https://github.com/michaeljancsy/conjuredsp-application/pull/332))** gemini-2.5-pro returned `"No capacity available for model gemini-2.5-pro on the server"` — distinct from the `QUOTA_EXHAUSTED` / "exhausted your capacity" strings the skill's old auto-fallback greps for. gemini-2.5-flash hit the same error. Only gemini-2.5-flash-lite had capacity. Rather than chase the next error string, the PR replaces the grep with a "dispatch once, the dispatching agent inspects the result event, decides" pattern that handles familiar and unfamiliar errors uniformly.
